...
namespace Game.Startup.Tests
{
    public class InkspellStartupTests
    {
        [Test]
        ...
        public IEnumerator LaunchSceneReachesMainAndReleasesView()
        {
            ...
            while (SceneManager.GetActiveScene().name != "Main" || Object.FindObjectOfType<LaunchPresentation>() != null)
            {
            ...
            var window = Object.FindObjectOfType<UIInkspellMainWindow>();
            Assert.That(window, Is.Not.Null);
            Assert.That(window.IsOpened, Is.True);
            var progress = (UIProgressBar)new SerializedObject(window).FindProperty("_generationProgress").objectReferenceValue;
            window.gameObject.SetActive(false);
            progress.SetValue(1, 1);
            ArchContext.Current.Event.Publish(new SpellGenerationChangedEvent());
            Assert.That(progress.NormalizedValue, Is.EqualTo(1), "停用窗口不应再处理业务事件。");
            window.gameObject.SetActive(true);
            progress.SetValue(1, 1);
            ArchContext.Current.Event.Publish(new SpellGenerationChangedEvent());
            var status = new UIInkspellMainWindowData().Status;
            Assert.That(progress.NormalizedValue, Is.EqualTo(Mathf.Clamp01(
        ...
        {
            EditorSceneManager.OpenScene("Assets/Game/Scenes/Launch.unity");
            Object.DestroyImmediate(Object.FindObjectOfType<GameEntry>());
            yield return new EnterPlayMode();
            ExerciseFailureView();
            yield return null;
            yield return new ExitPlayMode();
        ...
        {
            var view = Object.FindObjectOfType<LaunchPresentation>();
